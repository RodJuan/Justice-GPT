...
Justice GPT is engineered to detect and analyze the five primary causes of wrongful convictions identified by the Innocence Project. Our modular architecture systematically addresses these failures:
    ...
    2. Misapplication of Forensic Science: Forensic-Validator audits historical forensic data against modern peer-reviewed scientific standards.

    3. False Confessions: Psych-Layer (Active) utilizes NLP to detect coercion patterns and stress-related linguistic markers.

    4. Official Misconduct: Sentinel-Audit recognizes patterns of Brady violations (withheld evidence) and procedural anomalies.

    5. Coerced Pleas: Plea-Analyzer evaluates the disparity between available evidence and the pressure applied to waive trial rights.

🚀 Technical Stack & Modules

    AI/ML: Python, PyTorch, Transformers (BERT), Vision Transformers (TrOCR).

    Analysis: xAI Semantic API for legal contradiction detection.

    Psych-Layer (New!): BERT-based detector for identifying coerced confessions and linguistic patterns of false statements.

    Privacy: Zero-Knowledge Proofs (ZK-Proofs) for identity protection.

    Infrastructure: IPFS (storage) & Polygon (audit trails).

📂 Repository Structure

    /psych-layer: BERT model fine-tuned for coercion detection (Current Focus).

    /ocr-models: Vision Transformers for handwriting-to-data conversion.

    /api: FastAPI backend for legal auditing and scoring.

    /justice-nodes: P2P layer for decentralized, tamper-proof verification.
...
Phase 2: Intelligence & NLP (Q2 - Active) ⚙️

    [x] Integrate BERT-based models for false confession detection (Psych-Layer v1.0).

    [x] Deploy Vision Transformers (TrOCR) for handwritten letter processing.
